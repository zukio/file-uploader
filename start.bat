...
set path_project=%~dp0dist\win-unpacked\file-uploader.exe

REM # ウインドウタイトルで重複起動を確認（ブラウザのウインドウタイトル）
tasklist /FI "WINDOWTITLE eq MainWindow" | findstr "[0-9]" >NUL
if %ERRORLEVEL% == 1 (
 	start /min "MainWindow" "%path_project%" --plugins=%~dp0dist\win-unpacked\plugins --readDir=%USERPROFILE%/Pictures --saveDir=%~dp0plugins
